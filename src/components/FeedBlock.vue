  ...
  setup() {
    ...
    const initObserver = () => {
      observer.value = new IntersectionObserver(([entry]) => {
        if (entry.isIntersecting && !isLastPage.value) loadPosts();
      }, { threshold: 1.0 });
    ...
    const toggleLike = async (post) => {
      try {
        const res = await axios.put(`/api/feed/${post.postId}/like`, null, {
          headers: { Authorization: `Bearer ${localStorage.getItem("jwt")}` }
        });
        const likedNow = res.data;
        post.liked = likedNow;
        post.likeCount += likedNow ? 1 : -1;
      } catch (e) {
    ...
    const setCommentScrollEl = (post, el) => {
      post.commentScrollEl = el;
    };
    ...
    const submitComment = async (post) => {
      ...
      try {
        await axios.post(`/api/feed/${post.postId}/comment`, {
